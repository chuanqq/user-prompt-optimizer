 ...
 * With no timeoutMs or <=0, only the user signal is used; otherwise AbortSignal.any merges them.
 * The timeout fires via the timeout signal and does not pollute the user signal (so the caller can distinguish user cancel vs timeout).
 */
export function mergeAbortSignal(
  ...
  if (userSignal) return (AbortSignal as unknown as { any: (s: AbortSignal[]) => AbortSignal }).any([userSignal, timeoutSignal]);
