...
export async function runOptimize(deps: OptimizeDeps): Promise<void> {
  ...
  try {
    ...
    const roleOpts = { roleEnabled: settings.roleInjectionEnabled, roleHint: settings.roleHint };
    const user =
      mode === "lite"
        ? assembleLiteUserMessage(selection, roleOpts)
        : assembleUserMessage(templateContent, selection, roleOpts);
