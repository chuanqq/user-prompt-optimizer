...
- **Optional role injection**: when enabled, optimized prompts begin with a domain role line (e.g. "You are a senior backend specialist, facing the following task/requirement:") so the optimized prompt is ready to hand to another model. The domain is auto-inferred from the prompt's subject matter, or you can pin a fixed domain hint. Applies to both modes
