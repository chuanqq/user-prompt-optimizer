...
- **可选角色注入**：开启后，优化后的 prompt 会以一行领域角色开头（如「你是一名资深后端开发专家，面临以下任务/需求：」），便于直接交给另一个模型使用。领域由引擎根据 prompt 主题自动推断，也可在设置里固定一个领域提示。两种模式都生效
...
- **Vault 文件列举**：添加 `local` 来源模板时，插件会在选择器中列出 vault 内的 `.md` 文件路径供你挑选。只有你选中的那个路径会被读取（用于加载该模板）；其他文件内容不会被访问、传输或存到 vault 之外。
...
4. 在 Obsidian 中：设置 → 第三方插件 → 开启「社区插件」→ 启用「User Prompt Optimizer」

## 使用

1. 打开设置页「User Prompt Optimizer」，填入 provider 的 Base URL / API Key / Model（可点「获取模型」拉取），按需调整推理强度、采样参数、最大 token 数与请求超时
2. 在笔记中选中一段 prompt
